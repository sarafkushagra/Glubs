    ...
    <div className="min-h-screen bg-gradient-to-br from-black via-gray-900 to-indigo-800 relative">
      ...
      <div className="container mx-auto px-4 py-10 relative z-10">
        ...
        <div className="text-center mb-12 pt-16">
          <h1 className="text-5xl md:text-7xl font-extrabold text-white mb-4 h-20 flex items-center justify-center">
          ...
          <div className="max-w-3xl mx-auto flex flex-col md:flex-row gap-4 items-center justify-center mb-6">
            <Input
              ...
              className="md:w-1/2 bg-gray-900/70 border-gray-700 text-white placeholder-gray-400 focus:border-indigo-500 focus:ring-indigo-500/20 rounded-xl backdrop-blur-sm"
            />
            <Button
            onClick={() => navigate('/clubs/add')}
              className="bg-gradient-to-r from-indigo-600 to-purple-600 text-white font-semibold rounded-full px-6 py-3 shadow-lg hover:scale-105 hover:from-indigo-700 hover:to-purple-700 transition-all duration-300 border-none"
          >
            + Add Club
            </Button>
          </div>
        ...
        <div className="flex justify-end mb-4">
          <Button
            ...
            className={`flex items-center gap-2 px-4 py-2 rounded-md font-semibold text-sm transition-all duration-200 ${showOnlyFavorites ? 'bg-pink-600 text-white' : 'bg-gray-800 text-gray-300 hover:bg-gray-700'}`}
          >
            <Heart className={`h-5 w-5 ${showOnlyFavorites ? 'fill-pink-500 text-pink-200' : 'text-gray-400'}`} />
        ...
        <div className="grid gap-8 grid-cols-1 md:grid-cols-2 lg:grid-cols-3">
          {filteredClubs.length > 0 ? (
            filteredClubs.map((club, idx) => (
              <div key={club._id} className="relative group fade-in" style={{ animationDelay: `${idx * 80}ms` }}>
                <Card className="bg-gray-900/60 backdrop-blur-lg shadow-xl rounded-md transition-all duration-300 hover:shadow-2xl hover:shadow-indigo-500/10 transform hover:-translate-y-2 cursor-pointer overflow-hidden p-6">
                  ...
                  <p className="text-gray-300 line-clamp-3 mb-4 min-h-[48px]">{club.description}</p>
                  ...
                  <div className="flex gap-2 pt-2">
                    <Button
                      className={`px-3 py-1 rounded-md text-xs font-semibold flex items-center gap-1 ${favorites.includes(club._id) ? 'bg-pink-600 hover:bg-pink-700 text-white' : 'bg-gray-800 hover:bg-gray-700 text-gray-300'}`}
                      onClick={() => toggleFavorite(club._id)}
                    >
                      <Heart className={`h-4 w-4 ${favorites.includes(club._id) ? 'fill-pink-500 text-pink-200' : 'text-gray-400'}`} />
