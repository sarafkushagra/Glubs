...
const TeamRoomPage = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      // Fetch event details
      const eventRes = await axios.get(`http://localhost:3000/event/${eventId}`, { withCredentials: true })
      setEvent(eventRes.data.event)

      // Fetch user's team if exists
      try {
        const teamRes = await axios.get(`http://localhost:3000/teams/user/${userRes.data.user._id}/event/${eventId}`, {
      ...
      const requestsRes = await axios.get(`http://localhost:3000/teams/requests/${eventId}`, {
        withCredentials: true,
      })
      setTeamRequests(requestsRes.data.requests)

      // Fetch sent requests
      const sentRes = await axios.get(`http://localhost:3000/teams/sent-requests/${eventId}`, {
        withCredentials: true,
      })
  ...
  const updateTeam = async () => {
    ...
    try {
      await axios.put(
        `http://localhost:3000/teams/${userTeam._id}`,
        {
          name: teamName,
          description: teamDescription,
        },
  ...
  const sendTeamRequest = async (targetUserId, message = "") => {
    ...
    if (userTeam.leader._id !== user._id) {
      alert("Only team leaders can send invitations!")
      return
    }
    ...
    try {
      ...
      await axios.post(
        `http://localhost:3000/teams/request`,
        {
          teamId: userTeam._id,
          targetUserId,
          eventId,
  ...
  const leaveTeam = async () => {
    ...
    try {
      await axios.delete(`http://localhost:3000/teams/${userTeam._id}/member/${user._id}`, {
        withCredentials: true,
      })
      setUserTeam(null)
      fetchData()
  ...
  const removeMember = async (memberId) => {
    ...
    try {
      await axios.delete(`http://localhost:3000/teams/${userTeam._id}/member/${memberId}`, {
        withCredentials: true,
      })
      fetchData()
  ...
  const registerTeam = async () => {
    if (!userTeam) {
      alert("You need to be in a team to register!")
      return
    }

    if (userTeam.leader._id !== user._id) {
      alert("Only team leaders can register the team!")
      return
    }

    if (userTeam.members.length < event.teamMin) {
      alert(`Your team needs at least ${event.teamMin} members to register!`)
      return
    }

    try {
      await axios.post(
        `http://localhost:3000/event/${eventId}/register-team`,
        { teamId: userTeam._id },
        { withCredentials: true },
      )
  ...
  return (
    <div className={themeClasses.background}>
      ...
      <div className="fixed inset-0 overflow-hidden pointer-events-none">
        ...
        <div className="absolute top-3/4 right-1/4 w-96 h-96 bg-purple-500/10 rounded-full blur-3xl animate-pulse animation-delay-1000"></div>
        <div className="absolute top-1/2 left-1/2 w-80 h-80 bg-pink-500/10 rounded-full blur-3xl animate-pulse animation-delay-2000"></div>
      </div>
                ...
                <div className="gradient-border-inner">
                        ...
                        <div className="flex items-center gap-4">
                          ...
                          <div>
                            ...
                            <div className={`text-sm ${themeClasses.textMuted}`}>
                              {member.yearOfStudy} • {member.department}
                            </div>
                          </div>
                        </div>
                  ...
                  <div className="space-y-3">
                    ...
                    {userTeam.leader._id !== user._id && (
                      <button
                        onClick={leaveTeam}
                        className={`w-full py-3 rounded-xl font-semibold transition-all ${themeClasses.dangerButton}`}
                      >
                      ...
                      return (
                        ...
                        >
                          ...
                          {userTeam && userTeam.leader._id === user._id && userTeam.members.length < event.teamMax ? (
                            ...
                            <button
                              disabled
                              className="w-full py-3 rounded-xl font-semibold transition-all bg-gray-600/50 border border-gray-600/30 text-gray-400 cursor-not-allowed"
                            >
                              {!userTeam
                                ? "Create Team First"
                                : userTeam.leader._id !== user._id
...
//               </div>
//               <div>
//                 <h1 className={`text-4xl md:text-5xl font-bold ${themeClasses.text} mb-2`}>Team Room</h1>
//                 <div className="h-1 w-32 bg-gradient-to-r from-indigo-500 to-purple-500 rounded-full mx-auto"></div>
//               </div>
//             </div>

//             <p className={`${themeClasses.textSecondary} text-lg md:text-xl mb-6 max-w-2xl mx-auto`}>{event.title}</p>
...
//                 <div className="gradient-border-inner">
//                   <div className="flex items-center justify-between mb-6">
//                     <h2 className={`text-xl md:text-2xl font-bold ${themeClasses.text} flex items-center gap-3`}>
//                       <div className="p-2 bg-gradient-to-r from-indigo-500 to-purple-500 rounded-lg">
...
//                   <div className="mb-6">
//                     <div className="flex items-center justify-between mb-2">
//                       <h3 className={`text-lg md:text-xl font-bold ${themeClasses.text}`}>{userTeam.name}</h3>
//                       {userTeam.leader._id === user._id && (
//                         <button
//                           onClick={() => setShowEditTeam(true)}
//                           className={`p-2 ${themeClasses.button} border rounded-lg transition-all hover:scale-105`}
...
//                       <div
//                         key={member._id}
//                         className={`flex items-center justify-between p-4 ${isDarkMode ? "bg-gray-800/30" : "bg-white/50"} rounded-xl border ${isDarkMode ? "border-gray-700/50" : "border-gray-200"} transition-all hover:scale-105`}
//                       >
//                         <div className="flex items-center gap-3">
//                           <div
//                             className={`w-10 md:w-12 h-10 md:h-12 rounded-full bg-gradient-to-r ${
//                               index % 3 === 0
//                                 ? "from-indigo-500 to-purple-500"
//                                 : index % 3 === 1
//                                   ? "from-purple-500 to-pink-500"
...
//                             <div className={`font-semibold ${themeClasses.text} flex items-center gap-2`}>
//                               {member.username}
//                               {member._id === userTeam.leader._id && <Crown className="w-4 h-4 text-yellow-400" />}
//                             </div>
...
//                   <div className="space-y-3 mb-6">
//                     <div
//                       className={`flex items-center justify-between p-3 ${isDarkMode ? "bg-gray-800/30" : "bg-white/50"} rounded-lg`}
//                     >
//                       <span className={themeClasses.textMuted}>Members</span>
//                       <span className={`font-bold ${themeClasses.text}`}>
//                         {userTeam.members.length}/{event.teamMax}
//                       </span>
//                     </div>
//                     <div
//                       className={`flex items-center justify-between p-3 ${isDarkMode ? "bg-gray-800/30" : "bg-white/50"} rounded-lg`}
...
//                           userTeam.members.length >= event.teamMin ? "text-green-400" : "text-amber-400"
//                         }`}
//                       >
//                         {userTeam.members.length >= event.teamMin ? "Ready" : "Need More Members"}
//                       </span>
//                     </div>
//                   </div>
...
//                   {/* Action Buttons */}
//                   <div className="space-y-3">
//                     {userTeam.leader._id === user._id && userTeam.members.length >= event.teamMin && (
//                       <button
//                         onClick={registerTeam}
//                         className={`w-full py-4 rounded-xl font-bold transition-all transform hover:scale-105 ${themeClasses.successButton}`}
//                       >
//                         <CheckCircle className="w-5 h-5 inline mr-2" />
//                         Register Team for Event
//                       </button>
...
//             ) : (
//               /* Create Team Card */
//               <div className={`${themeClasses.card} border rounded-2xl p-6 md:p-8 shadow-2xl`}>
//                 <div className="text-center mb-6">
//                   <div className="w-16 md:w-20 h-16 md:h-20 mx-auto mb-4 bg-gradient-to-r from-indigo-500 to-purple-500 rounded-full flex items-center justify-center">
//                     <Plus className="w-8 md:w-10 h-8 md:h-10 text-white" />
//                   </div>
//                   <h2 className={`text-xl md:text-2xl font-bold ${themeClasses.text} mb-2`}>Create Your Team</h2>
//                   <p className={`${themeClasses.textMuted} text-sm md:text-base`}>
//                     Start building your dream team for this event
//                   </p>
//                 </div>
//                 <button
//                   onClick={() => setShowCreateTeam(true)}
//                   className={`w-full py-4 rounded-xl font-bold transition-all transform hover:scale-105 ${themeClasses.primaryButton}`}
//                 >
...
//                 <h3 className={`text-lg md:text-xl font-bold ${themeClasses.text} mb-4 flex items-center gap-2`}>
//                   <MessageSquare className="w-5 md:w-6 h-5 md:h-6 text-indigo-400" />
...
//                     <div
//                       key={request._id}
//                       className={`p-4 ${isDarkMode ? "bg-gray-800/30" : "bg-white/50"} rounded-xl border ${isDarkMode ? "border-gray-700/50" : "border-gray-200"}`}
//                     >
//                       <div className="flex items-center justify-between mb-3">
//                         <div>
//                           <div className={`font-semibold ${themeClasses.text}`}>{request.team.name}</div>
//                           <div className={`text-sm ${themeClasses.textMuted}`}>from {request.team.leader.username}</div>
//                         </div>
//                         <div className={`text-xs ${themeClasses.textMuted} flex items-center gap-1`}>
//                           <Clock className="w-3 h-3" />
//                           {new Date(request.createdAt).toLocaleDateString()}
//                         </div>
//                       </div>
//                       {request.message && (
//                         <p className={`text-sm ${themeClasses.textSecondary} mb-3 italic`}>"{request.message}"</p>
//                       )}
//                       <div className="flex gap-2">
//                         <button
//                           onClick={() => respondToRequest(request._id, "accept")}
//                           className={`flex-1 py-2 px-3 rounded-lg font-semibold transition-all ${themeClasses.successButton} text-sm`}
//                         >
//                           Accept
//                         </button>
//                         <button
//                           onClick={() => respondToRequest(request._id, "reject")}
//                           className={`flex-1 py-2 px-3 rounded-lg font-semibold transition-all ${themeClasses.dangerButton} text-sm`}
//                         >
//                           Decline
//                         </button>
//                       </div>
...
//                   </div>
//                   Find Teammates ({filteredUsers.length})
//                 </h2>
//                 <button
//                   onClick={() => setShowFilters(!showFilters)}
//                   className={`flex items-center gap-2 px-4 py-2 ${themeClasses.button} border rounded-lg transition-all hover:scale-105`}
//                 >
...
//                     value={searchTerm}
//                     onChange={(e) => setSearchTerm(e.target.value)}
//                     className={`w-full pl-12 pr-4 py-3 ${themeClasses.input} rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition-all`}
//                   />
//                 </div>
...
//                           value={filterYear}
//                           onChange={(e) => setFilterYear(e.target.value)}
//                           className={`w-full p-3 ${themeClasses.input} rounded-lg focus:ring-2 focus:ring-indigo-500 transition-all`}
//                         >
//                           {years.map((year) => (
//                             <option key={year} value={year}>
//                               {year}
...
//                   <div
//                     className={`w-20 h-20 mx-auto mb-6 ${isDarkMode ? "bg-gray-800/30" : "bg-gray-100"} rounded-full flex items-center justify-center`}
//                   >
//                     <Users className={`w-10 h-10 ${themeClasses.textMuted}`} />
//                   </div>
//                   <h3 className={`text-xl font-semibold ${themeClasses.textSecondary} mb-2`}>No users found</h3>
//                   <p className={themeClasses.textMuted}>Try adjusting your search or filters</p>
//                 </div>
//               ) : (
//                 <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 max-h-96 overflow-y-auto hide-scrollbar">
//                   {filteredUsers.map((availableUser, index) => {
//                     const hasSentRequest = sentRequests.some((req) => req.targetUser._id === availableUser._id)
//                     return (
//                       <div
//                         key={availableUser._id}
//                         className={`p-4 ${themeClasses.card} border rounded-xl transition-all hover:scale-105 ${themeClasses.cardHover} shadow-lg`}
//                       >
...
//                           </div>
//                           <div className="flex-1">
//                             <div className={`font-semibold ${themeClasses.text}`}>{availableUser.username}</div>
//                             <div className={`text-sm ${themeClasses.textMuted}`}>{availableUser.email}</div>
//                           </div>
//                         </div>

//                         <div className={`text-sm ${themeClasses.textSecondary} mb-3 space-y-1`}>
//                           {availableUser.yearOfStudy && (
//                             <div className="flex items-center gap-2">
//                               <GraduationCap className="w-4 h-4 text-indigo-400" />
//                               {availableUser.yearOfStudy}
//                             </div>
...
//                           }}
//                           disabled={!userTeam || userTeam.leader._id !== user._id || hasSentRequest}
//                           className={`w-full py-2 px-4 rounded-lg font-semibold transition-all ${
//                             hasSentRequest
//                               ? isDarkMode
//                                 ? "bg-gray-700 text-gray-400 cursor-not-allowed"
//                                 : "bg-gray-200 text-gray-500 cursor-not-allowed"
//                               : !userTeam || userTeam.leader._id !== user._id
//                                 ? isDarkMode
//                                   ? "bg-gray-800 text-gray-500 cursor-not-allowed"
//                                   : "bg-gray-100 text-gray-400 cursor-not-allowed"
//                                 : themeClasses.primaryButton + " transform hover:scale-105"
//                           }`}
//                         >
//                           {hasSentRequest ? (
//                             <>
...
//           <div className={`${themeClasses.card} border rounded-2xl p-6 md:p-8 w-full max-w-md shadow-2xl`}>
//             <div className="flex items-center justify-between mb-6">
//               <h3 className={`text-xl md:text-2xl font-bold ${themeClasses.text} flex items-center gap-3`}>
//                 <div className="p-2 bg-gradient-to-r from-indigo-500 to-purple-500 rounded-lg">
...
//               <div>
//                 <label className={`block text-sm font-medium ${themeClasses.textSecondary} mb-2`}>Team Name *</label>
//                 <input
//                   type="text"
//                   value={teamName}
//                   onChange={(e) => setTeamName(e.target.value)}
...
//                   rows={3}
//                   className={`w-full p-3 ${themeClasses.input} rounded-lg focus:ring-2 focus:ring-indigo-500 transition-all resize-none`}
//                 />
//               </div>
//               <div className="flex gap-3 pt-4">
//                 <button
//                   onClick={() => setShowCreateTeam(false)}
//                   className={`flex-1 py-3 px-4 ${themeClasses.button} border rounded-lg font-semibold transition-all`}
//                 >
//                   Cancel
//                 </button>
//                 <button
//                   onClick={createTeam}
//                   className={`flex-1 py-3 px-4 rounded-lg font-semibold transition-all ${themeClasses.primaryButton}`}
//                 >
//                   Create Team
//                 </button>
//               </div>
...
//               <h3 className={`text-xl md:text-2xl font-bold ${themeClasses.text} flex items-center gap-3`}>
//                 <div className="p-2 bg-gradient-to-r from-indigo-500 to-purple-500 rounded-lg">
//                   <Edit3 className="w-5 md:w-6 h-5 md:h-6 text-white" />
//                 </div>
//                 Edit Team
//               </h3>
//               <button
...
//               <h3 className={`text-xl md:text-2xl font-bold ${themeClasses.text} flex items-center gap-3`}>
//                 <div className="p-2 bg-gradient-to-r from-purple-500 to-pink-500 rounded-lg">
//                   <Send className="w-5 md:w-6 h-5 md:h-6 text-white" />
//                 </div>
//                 Send Invitation
//               </h3>
//               <button
...
//                 </div>
//                 <div>
//                   <div className={`font-semibold ${themeClasses.text}`}>{selectedUser.username}</div>
//                   <div className={`text-sm ${themeClasses.textMuted}`}>{selectedUser.email}</div>
//                 </div>
//               </div>
//             </div>
